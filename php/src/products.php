...
$products = $conn->query(
    "SELECT * FROM tb_products WHERE product_status = 1 ORDER BY product_id DESC"
);
$productsData = [];
                        ...
                        <tbody>
                            ...
                            foreach ($productsData as $key => $product) { ?>
                                <tr>
                                    ...
                                    <td>
                                        <button data-toggle="modal" data-target="#addProductSystemModal" onclick="editProduct('<?php echo $product->product_id; ?>','<?php echo $product->product_name; ?>','<?php echo $product->product_price; ?>','<?php echo $product->product_size; ?>','<?php echo $product->unit_id; ?>','<?php echo $product->category_id; ?>','<?php echo $product->product_type; ?>')" class="btn btn-primary" type="button">แก้ไข</button>
                ...
                <form class="product-system-validation" action="action/ac_product.php?ac=createProduct" id="addProductSystem" novalidate="">
                    <div class="modal-body">
                        ...
                        <div class="form-row">
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="validationCustom01">หมวดหมู่สินค้า</label>
                                <select name="productType"  class="custom-select" required="" id="productType" >
                                    <option value="">เลือกหมวดหมู่</option>
                                    <option value="ขายปลีก">ขายปลีก</option>
                                    <option value="ขายส่ง">ขายส่ง</option>
                                </select>
                                <div class="invalid-feedback">โปรดเลือกหมวดหมู่สินค้า</div>
                            </div>
                        </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-row">
                            <div class="col-md-12 mb-3">
                                ...
                                <input name="userBy" class="form-control" value="<?php echo $_SESSION[
                                    "user_id"
                                ]; ?>" hidden id="userBy" type="text" placeholder="">
...
<script>
    function deleteProductStock(plId, productId) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `action/ac_product.php?ac=deleteProductStock&plId=${plId}&productId=${productId}&userBy=<?php echo $_SESSION[
                        "user_id"
                    ]; ?>`,
                    type: "POST",
                    data: {
    ...
    function editProduct(productId, productName, productPrice, productSize, productUnit, productCategory,productType) {
    ...
    function deleteProductSystem(productId) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `action/ac_product.php?ac=deleteProduct&productId=${productId}&userBy=<?php echo $_SESSION[
                        "user_id"
                    ]; ?>`,
                    type: "POST",
                    data: {
