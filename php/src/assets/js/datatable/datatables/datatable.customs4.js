$(document).ready(function () {
  ...
  var paymentRow = $("#payment-datable").DataTable({
    columnDefs: [
      {
        targets: 0,
        searchable: false,
  ...
  $("#payment-datable tbody").on(
    "change",
    'input[type="checkbox"]',
    function () {
      // If checkbox is not checked
      if (!this.checked) {
        var el = $("#payment-select-all").get(0);
  ...
  $("#payment-select-all").on("click", function () {
    ...
    var checkboxes = paymentRow.column(0).nodes();
    let isCheckAll = $(this).prop("checked");
    $('input[type="checkbox"]', checkboxes).prop("checked", isCheckAll);
  });

  // $("#payment-datable tbody").on("click", "tr", function () {
  //   $(this).toggleClass("selected");
  // });

  var total = [];

  function calculateTotal(event, price) {
    let idx = event.target.getAttribute("index");

    return { idx: idx, data: event.target.value * price };
  }
  ...
  $("#payment-btn").on("click", function () {
    ...
    $('input[type="checkbox"]:checked', paymentRow.column(0).nodes()).each(
      function () {
    ...
    selectedData.forEach(function (dataItem, key) {
      // Create a new form-row div with col-md-6 columns
      ...
      productAmountInput.on("change", function (event) {
        let { idx, data } = calculateTotal(event, dataItem[3]);
        total[idx] = data;

        let summary = total.reduce((prev, next) => prev + next);

        $("#total").text(summary + " บาท");
        $("#totalProduct").val(summary);
      });
      ...
      col2Div.append(productAmountInput, productId, invalidFeedbackDiv);

      // Append col1Div and col2Div to formRowDiv
      formRowDiv.append(col1Div, col2Div);

      // Append the formRowDiv to the document or another container element
      $("#addPayment .modal-body").append(formRowDiv);
    });

    let summaryContainer = $('<div class="form-row mb-3"></div>');
    let summaryDiv = $('<div class="col-md-6 mb-3"><h5>รวม:</h5></div>');
    let summaryTotal = $(
      `<div class="col-md-6 mb-3"><h5 id="total"> ${
        total.length > 0
          ? total.parseInt().toLocaleString("en-US", {
    ...
    if (selectedData.length)
      $("#addPayment .modal-body").append(summaryContainer);
  });
